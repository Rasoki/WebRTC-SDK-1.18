...
/*
   Tutorial 
   Phone prototype with OAuth2 authentication
...
let authServerConfig; // Default from site setting.
let keycloak = null; // Keycloak server adapter
let authLoginPostponed = false; // updateToken is failed, re-login postponed not to interrupt active call.
let authTests = ''; // For SBC testing. Use wrong access token, or wrong user name.
// To disable testing use empty string value.
// See description of all tests in the tutorial.html
...
function documentIsReady() {
    // Load configurations
    authServerConfig = DefaultAuthServerConfig; // don't allow user change the configuration via GUI
...
function startPhone() {
    ...
    ac_log(`AudioCodes WebRTC SDK. Phone with OAuth2`);
...
function initializeAuthServer() {
    ...
    keycloak.init({ onLoad: 'login-required' })
        ...
        .then(() => {
            // To disable auto call login() if updateToken() is failed.
            keycloak.loginRequired = false;

            ac_log('keycloak: profile:', keycloak.profile);
            userAccount = {
            ...
            authTests = sessionStorage.getItem('authTests'); // load authTests from local storage.
            if (authTests === null) {
                authTests = '';
            } else {
                ac_log('Warning: USED PHONE TEST MODE: authTests=\'' + authTests + '\'');
            }

            if (authTests.includes('user')) { // Let use wrong user name.
                userAccount.user = 'jonhDow';
            }
            if (authTests.includes('r0')) {
                phone.setOAuthToken(null); // Don't include Authorization header to the 1st REGISTER
            } else if (authTests.includes('r1')) {
                phone.setOAuthToken('wrong_access_token_value'); // Include wrong token value to the 1st REGISTER
            }
        ...
        .catch((err) => {
            ...
            guiError('auth.server initialization error' + err);
...
function initSipStack(account) {
    ...
    phone.setAccount(userAccount.user, userAccount.displayName, userAccount.password);
    ...
    phone.setUserAgent(`AudioCodes WebRTC SDK. Phone with OAuth2 ${phone.version()} ${phone.getBrowserName()}`);
    ...
    phone.setListeners({
        loginStateChanged: function (isLogin, cause, response) {
            switch (cause) {
                ...
                case "disconnected":
                    ...
                    break;
                case "login failed":
                    let statusCode = response !== null ? response.status_code : 0;
                    ac_log('phone>>> loginStateChanged: login failed. Status code=' + statusCode);
                    guiError('SBC server: login failed');

                    if (statusCode == 401) {
                        ac_log('Auth token is wrong.');
                        updateAuthToken()
                            .then(() => {
                                if (!authLoginPostponed)
                                    phone.login();
                            });
                    } else {
                        if (activeCall === null) // if no active call
        ...
        callTerminated: function (call, message, cause, redirectTo) {
            ...
            ac_log('phone>>> call terminated callback, cause=' + cause + (statusCode != 0 ? ', response status code=' + statusCode : ''));
            ...
            if (cause === 'Redirected') {
                ...
                ac_log('Auth token is wrong.');
                guiWarning('Authentication problem.');
                updateAuthToken()
                    .then(() => {
                        // start of test code
                        authTests = authTests.replace(/i[01]/, ''); // To prevent endless loop, remove i? tests.
                        // end of test code

                        // To prevent endless loop with authentication problem, set flag in call data.
                        if (call.data['try_fix_auth_problem'] === undefined) {
                            let user = call.data['_user'];
                            guiMakeCallTo(user, call.hasSendVideo() ? phone.VIDEO : phone.AUDIO, null, { 'try_fix_auth_problem': true });
                        } else {
                            ac_log('Authentication problem did not fixed');
                        }
                    });
            } else if (authLoginPostponed) {
                authLoginPostponed = false;
                ac_log('keycloak: login()');
                keycloak.login();
            } else if (call.isOutgoing() && !call.wasAccepted() && statusCode >= 500 && statusCode < 600 && phoneConfig.switchSbcAtInvite5xx) {
...
function guiMakeCallTo(callTo, videoOption, extraHeaders = null, extraData = null) {
    ...
    // start of test code
    if (authTests.includes('i0')) {
        phone.setOAuthToken(null); // Don't include Authorization header to initial INVITE
    } else if (authTests.includes('i1')) { // Set wrong token value to initial INVITE
        phone.setOAuthToken('wrong_access_token_value');
    }
    // end of test code
