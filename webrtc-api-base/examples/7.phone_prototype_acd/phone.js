...
/*
   Tutorial
   Phone with automatic call distribution
   ...
   Added:
   - BroadSoft ACD (automatic call distribution).
...
let acdAgent = new BroadsoftAcdAgent();
...
function startPhone() {
    ...
    ac_log(`AudioCodes WebRTC SDK. Phone with ACD`);
...
function initSipStack(account) {
    ...
    // For SUBSCRIBE can be used different authentication.
    //phone.addCredential({realm:'xxx', password: 'xxx', username: 'xxx'});
    ...
    phone.setUserAgent(`AudioCodes WebRTC SDK. Phone with ACD ${phone.version()} ${phone.getBrowserName()}`);
...
function guiInit() {
    ...
    //------------ ACD buttons ---------
    document.getElementById('acd_btn').onclick = acdPanel;
    document.getElementById('acd_return_btn').onclick = acdReturns;
    document.getElementById('acd_save_account_btn').onclick = acdStoreAccount;
    document.getElementById('acd_start_btn').onclick = acdStart;
    document.getElementById('acd_stop_btn').onclick = acdStop;
    document.getElementById('acd_logon_btn').onclick = acdLogon;
    document.getElementById('acd_logoff_btn').onclick = acdLogoff;
    document.getElementById('acd_start_and_logon_btn').onclick = () => { acdStart(); acdLogon(); };
    document.getElementById('acd_available_btn').onclick = acdAvailable;
    document.getElementById('acd_unavailable_btn').onclick = acdUnavailable;
    document.getElementById('acd_working_after_call_btn').onclick = acdWorkingAfterCall;
    document.getElementById('states_clear_btn').onclick = acdClearLog;
...
function acdPanel() {
    if (userAccount.acdUser) {
        document.querySelector('#acd_account_form [name=acd_user]').value = userAccount.acdUser;
        document.querySelector('#acd_account_form [name=acd_password]').value = userAccount.acdPassword;
        document.querySelector('#acd_account_form [name=acd_expires]').value = userAccount.acdExpires;
    }
    guiEnableSound();
    guiInfo('');
    guiShowPanel('acd_panel');
    acdAgent.setLog(ac_log);
    acdAgent.setListeners(acdState, acdError);
}
...
function acdStoreAccount() {
    userAccount.acdUser = document.querySelector('#acd_account_form [name=acd_user]').value.trim();
    userAccount.acdPassword = document.querySelector('#acd_account_form [name=acd_password]').value.trim();
    userAccount.acdExpires = document.querySelector('#acd_account_form [name=acd_expires]').value.trim();
    if (userAccount.acdUser !== '') {
        guiStoreAccount(userAccount);
    } else {
        ac_log('Warning: no acd user');
    }
}
...
function acdStart() {
    ...
    userAccount.acdExpires = document.querySelector('#acd_account_form [name=acd_expires]').value.trim();
    if (userAccount.acdUser === '') {
        guiWarning('Please fill ACD account');
        return;
    }
    guiStoreAccount(userAccount);
    acdAgent.setAccount(userAccount.acdUser, userAccount.acdPassword, parseInt(userAccount.acdExpires));

    // Set state that will be set during logon (ready or notReady)
    //acdAgent.setLoginState('ready');
    acdAgent.setLoginState('notReady', 110);
    guiWarning('');
    try {
        acdAgent.start();
    } catch (e) {
...
function acdAvailable() {
    guiWarning('');
    try {
        acdAgent.setState('ready');
    } catch (e) {
        guiWarning(e);
    }
}

function acdUnavailable() {
    let notReadyField = document.getElementById('acd_unavailable_code').value.trim();
    let notReadyCode = parseInt(notReadyField);
    guiWarning('');
    try {
        acdAgent.setState('notReady', notReadyCode);
    } catch (e) {
        guiWarning(e);
    }
}

function acdWorkingAfterCall() {
    guiWarning('');
    try {
        acdAgent.setState('workingAfterCall');
    } catch (e) {
        guiWarning(e);
    }
}

function acdClearLog() {
    let list = document.getElementById('states_ul');
    while (list.firstChild) {
        list.firstChild.remove();
    }
}
...
function acdLog(str) {
    let list = document.getElementById('states_ul');
    let li = document.createElement('li');
    li.innerHTML = createTimestamp() + ' ' + str;
    list.appendChild(li);
}

/**
 * logon success acdState: 'loggedOn' or not empty 'guestAddress'
 * if used wrong authentication guestAddress will be empty and there will not be 'loggedOn'
 * 
 * logout acdState: 'loggedOff' or empty guestAddress
 * 
 * 
 * user state: ready
 *             notReady [optional code]
 *             workingAfterCall
 * Can flick: notReady, notReady code or notReady ready.
 */
function acdState(state, substate) {
    ac_log('ACD state', state, substate);
    acdLog(`state: ${state}  ${substate !== undefined ? '"' + substate + '"' : ''} `);
}

/**
 * Hoteling or feature subscription termination. (if server is rebooted)
 * XML format or parsing error. (should not be)
 * Authentication error detected if the same nonce requested again.
 * 
 * err - error description
 * terminated - subscription terminated, and should 
 *   be restarted (stop/start)
 */
function acdError(err, terminated) {
    ac_log(`ACD error ${err} ${terminated ? ' [one of subscription is terminated]' : ''}`);
    acdLog(`error: ${err}`);
}
